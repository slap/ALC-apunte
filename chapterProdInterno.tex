...
\end{definicion}
Que ocurre si $\Ab\in \K^{n\times m}$? podremos definir algo similar?.  Volveremos sobre este caso m\'as adelante cuando definamos la \emph{pseudoinversa}.
\begin{observacion}
...
\item Para toda $\Qb$ unitaria/ortogonal, $\kappa(\Qb)=1$ para la norma $\|\cdot\|_2$ (ver item (2) en Proposición \ref{prop:unitarias}).
...
$\Pb\neq \cero$ y existe $\cero\neq\wb$ tal que $\vb=\Pb\wb\neq\cero$. Para ese $\vb$, $\Pb\vb=\vb\neq\cero$ y luego $\|\Pb\|_2\ge 1$, lo que implica $\|\Pb\|_2= 1$.

\noindent $(3)\Rightarrow (1)$: asumamos que existen $\vb\in \Im(\Pb)$ y
$\wb\in \Nu(\Pb)$ tales que $\vb^*\wb\neq 0$ y $\|\vb\|_2=1=\|\wb\|_2$. 
Definamos $\ub=\vb-(\wb^*\vb)\wb$. Se tiene que $\|\Pb\ub\|_2=\|\Pb\vb\|_2=\|\vb\|_2=1$, y como $\|\Pb\|_2=1$, resulta
\begin{align*}
1 &=\|\Pb\ub\|_2^2 \\
&\le \|\Pb\|_2^2 \|\ub\|_2^2 = \|\ub\|_2^2  = \langle \vb-(\wb^*\vb)\wb, \vb-(\wb^*\vb)\wb \rangle \\
&= \|\vb\|_2^2- \langle \vb,  (\wb^*\vb)\wb \rangle - \langle (\wb^*\vb)\wb,   \vb\rangle + \|(\wb^*\vb)\wb\|_2^2 \\
&= \|\vb\|_2^2- (\wb^*\vb)(\vb^*\wb)-\overline{(\wb^*\vb)}\wb^*\vb+|\wb^*\vb|^2\|\wb\|_2^2 \\
&= 1-\overline{(\vb^*\wb)}(\vb^*\wb)-\overline{(\wb^*\vb)}\wb^*\vb+|\wb^*\vb|^2 \\
&=1-|\vb^*\wb|^2-|\wb^*\vb|^2 + |\wb^*\vb|^2 \\
&=1-|\vb^*\wb|^2 <1,
\end{align*}

absurdo. Luego $\Im(\Pb)\perp \Nu(\Pb)$ y $\Pb$ es ortogonal.
 \end{proof}
